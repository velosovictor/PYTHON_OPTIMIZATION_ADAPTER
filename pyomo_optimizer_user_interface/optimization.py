# ============================================================================
# TENSOR-BASED OPTIMIZATION MODULE - REVOLUTIONARY APPROACH
# ============================================================================
# Pure tensor optimization: find optimal tensor A by tuning parameters B
# PARAMETER A: Target tensor (what we optimize FOR) - Results in optimal xarray
# PARAMETER B: Tuning variables (what we optimize WITH) - The knobs we turn
# 100% PROBLEM-AGNOSTIC - works for ANY domain: physics, finance, logistics, etc.
...
# ============================================================================
# TENSOR EXPRESSION PARSER - THE BRAIN OF PURE OPTIMIZATION
# ============================================================================
def parse_tensor_expression(expr_str, model):
    """
    Converts tensor expressions to Pyomo expressions
    
    SUPPORTED EXPRESSIONS:
    Point-wise: "x[0]", "x[-1]", "v[1]", "x[0] + v[-1]"
    Global: "sum(x**2)", "sum(abs(x))", "max(abs(x))", "sum(x)", "var(x)"
    Multi-param: "0.5*m*sum(v**2) + 0.5*k_eff*sum(x**2)"
    """
    
    print(f"🧠 Parsing tensor expression: '{expr_str}'")
    
    # Handle global sum expressions: sum(x**2), sum(abs(x)), etc.
    sum_pattern = r'sum\(([^)]+)\)'
    if 'sum(' in expr_str:
        return parse_global_sum_expression(expr_str, model)
    
    # Handle max expressions: max(abs(x)), max(x), etc.
    if 'max(' in expr_str:
        return parse_global_max_expression(expr_str, model)
    
    # Handle variance: var(x)
    if 'var(' in expr_str:
        return parse_variance_expression(expr_str, model)
    
    # Handle point-wise expressions: x[0], x[-1], v[1]
    if '[' in expr_str and ']' in expr_str:
        return parse_pointwise_expression(expr_str, model)
    
    # Handle simple parameter names: x, v, k_eff (auto-decide best metric)
    if expr_str.strip() in [f.func.__name__ for f in unknown_funcs]:
        return parse_auto_global_expression(expr_str.strip(), model)
    
    # Handle complex multi-parameter expressions
    return parse_complex_expression(expr_str, model)
